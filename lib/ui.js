...
function displayArrivals ( mode )
{
  ...
  /////////////////////////////////////////////////////////////////////////////
  case "dest":
    var lastLine = "";       // State variables to track what the
    var lastDest = "";       // last-used line/dest were to see if a 
                             // new group should be started.
    var crHeaderPrinted = false;

    /*
     * Loop through until there are no more array elements
     */
    for (var b = 0; b < masstrac.arrivals.length; b++) {
      
      var arrObj = masstrac.arrivals[b];
      var eta = Math.round( arrObj.Countdown / 60 );
      
      // Handle the subway lines ... they're guaranteed to be 
      // first because of the way I coded the sorting function
      
      if (arrObj.Line == "red" ||
          arrObj.Line == "blue" ||
          arrObj.Line == "orange" ) {
        if (arrObj.Destination != lastDest) {

          if (arrObj.Line != lastLine) {
            lastLine = arrObj.Line;
            if (b != 0)
              arrCont += "</ul></ul></ul><br />";
            
            arrCont += "<ul class='sortArrival'><li class='route ";
            
            // Because of the earlier conditon, it's guaranteed to be one of the
            // three possibly tracked lines.
            if (arrObj.Line == "red")
              arrCont += "redLine'>RED LINE";
            else if (arrObj.Line == "orange")
              arrCont += "orangeLine'>ORANGE LINE";
            else
              arrCont += "blueLine'>BLUE LINE";
          ...
          lastDest = arrObj.Destination;
          arrCont += "<li class='destination'>" + lastDest + "</li><ul>";
        } 
        
        // Depending on the "mode" the trip is in, draw the element
        arrCont += "<li class='countdown";
        
        if (arrObj.Type == "sch") {
          if (arrObj.Countdown <= 60) {
            arrCont += " scheduled'>Due</li>";
          } else {
            arrCont += " scheduled'>" + eta + " min</li>";
          }
          
        } else if (arrObj.Type == "pre") {
          arrCont += " predicted'>" + eta + " min</li>";
          
        } else if (arrObj.Type == "app") {
          arrCont += " approaching'>APPR</li>";
          
        } else if (arrObj.Type == "arr") {
          arrCont += " arriving'>ARRV</li>";
          
        } else if (arrObj.Type == "dep" || arrObj.Type == "brd") {
          arrCont += " departing'>DPRT</li>";
          
        } else {
          arrCont += " scheduled'>????</li>";
        }
        
      }
      
      // Otherwise it's a commuter rail trip (hopefully)
      else {
        
        /*
         * This ugly batch of code is to handle the fact that the sorting only
         * distinguishes between inbound / outbound trips, and thus displaying
         * outbound trips by just checking the "lastDest" results in them
         * clumping in weird ways. The lazy way out of this is simply to take
         * all the outbound trips (guaranteed to be after the inbound ones, if
         * any) and displays them in the order they're found (soonest arrival
         * estimate to latest).
         * 
         * First, initialize the content structure of the Commuter Rail Data
         * This includes setting the destination of the first-encountered
         * trip, since the sorting guarantees the inbound trips will be 
         * first if present at all.
         */
        if (!crHeaderPrinted) {
          if (b != 0)
            arrCont += "</ul></ul></ul><br />";
          
          // Just call this section "COMMUTER RAIL"
          arrCont += "<ul class='sortArrival'><li class='route commuterRail'>COMMUTER RAIL</li><ul>";
          
          /*
           * Only print the inbound destination if there are trips that way.
           * Fortunately, they'll always (?) be destined to either North or
           * South Station. Additionally, our sort-by-destination will always
           * place inbound trips before the outbound ones if any are present.
           */
          if (arrObj.Destination == "North Station" ||
              arrObj.Destination == "South Station"   ) {
            arrCont += "<li class='destination'>" + arrObj.Destination + "</li>";
          } else {
            arrCont += "</ul>";
          }
          
          crHeaderPrinted = true;    // Needed to avoid printing header every time
        }
        
        lastDest = arrObj.Destination;
        
        /*
         * Outbound trip detection and list initialization.
         */
        if (lastDest != "North Station" &&
            lastDest != "South Station"   ) {

          arrCont += "<ul><li class='destination outbound'>" + lastDest + "</li>";
        }

        /*
         * Depending on the "mode" the trip is in, 
         * draw the element with the corresponding CSS class.
         */
        arrCont += "<li class='countdown";

...
var options = {
  ...
  autoRef : false,
  hhmm : false,
  //times : false,   // Internet Explorer turned this into the times x char
                     // when it was inserted as the URL parameter "&times"
  verbose : false
};
...
{
  // The names as stored are a little quirky in order to facilitate 
  // proper name matching. However, this means there are weird spaces
  // particularly in station names with "/" in them. 
  // The MBTA signage is also all capitalized, so we will follow that convention.
  var prettyName = ((stationObject.name.split( "/" )).join( " / " )).toUpperCase();
  $("#nameArea").html( "<h1>" + prettyName + "</h1>" );
}
...
function timeToString( arrObj, countdowns )
{
  ...
  if (countdowns) {
    // Use Professor Heines' easier AM/PM time function
    arrival += jGetAMPMTime( new Date(arrObj.UNIXtime * 1000), false, false );
...
function reloadData()
{
   ...
   */
  displayArrivals( (options.verbose) ? "time" : "dest", true, options.hhmm );

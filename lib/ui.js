 ...
 * 
 * This only creates the <li>...</li> portion, so it's expected that the
 * return from this function is used within a <ul> structure.
 */
function makeCountdownOrb ( arrObj )
{
  var time = "<li class='countdown ";
  
  /**********************************************************
   * RESUME CODING HERE
   * 
   * Wed 20-Mar-2013 (12:45 pm)
   **********************************************************/
  
  if (arrObj.Type == "sch") {
    if (arrObj.Countdown <= 60)
      time += "scheduled'>Due</li>";
    else
      time += "scheduled'>" + Math.round( arrObj.Countdown / 60 ) + " min</li>";
  } else if (arrObj.Type == "pre") {
    time += "predicted'>" + Math.round( arrObj.Countdown / 60 ) + " min</li>";
  } else if (arrObj.Type == "app") {
    time += "approaching'>APPR</li>";
  } else if (arrObj.Type == "arr") {
    time += "arriving'>ARRV</li>";
  } else if (arrObj.Type == "dep") {
    time += "departing'>DPTR</li>";
  } else if (arrObj.Type == "brd") {
    time += "departing'>DPRT</li>";
  } else if (arrObj.Type == "del") {
    time += "predicted'>STOP</li>";
  } else {
    time += "scheduled'>????</li>";
  }  
  ...
  case "dest":
    ...
    if (masstrac.arrSorted.orange.f.length > 0 ||
      ...
      for (tripNo = 0; tripNo < masstrac.arrSorted.orange.f.length; tripNo++) {
        arrCont += makeCountdownOrb( masstrac.arrSorted.orange.f[tripNo] );
      }
      arrCont += "</ul>";
      
      arrCont += "<li class='destination'>Oak Grove</li><ul>";
      for (tripNo = 0; tripNo < masstrac.arrSorted.orange.o.length; tripNo++) {
        arrCont += makeCountdownOrb( masstrac.arrSorted.orange.o[tripNo] );
      }
      arrCont += "</ul>";
    ...
    if (masstrac.arrSorted.blue.b.length > 0 ||
      ...
      for (tripNo = 0; tripNo < masstrac.arrSorted.blue.b.length; tripNo++) {
        arrCont += makeCountdownOrb( masstrac.arrSorted.blue.b[tripNo] );
      }
      arrCont += "</ul>";
      
      arrCont += "<li class='destination'>Wonderland</li><ul>";
      for (tripNo = 0; tripNo < masstrac.arrSorted.blue.w.length; tripNo++) {
        arrCont += makeCountdownOrb( masstrac.arrSorted.blue.w[tripNo] );
      }
      arrCont += "</ul>";

...
masstrac.getCommuterRail = function( stop_name, line_name )
{
  ...
  /*
   * Go through the JSON again, but this time push the elements to their
   * respective TripID into masstrac.trips. (The CR data is so different that
   * it is unfeasible to try and push in conjunction with searching for trips.
   */
  for (c = 0; c < arrivalEntry.Messages.length; ++c) {
    for (var d = 0; d < masstrac.trips.length; ++d) {
      
      // Find if an entry exists with the trip name already
      if (arrivalEntry.Messages[c][1].Value == masstrac.trips[d].TripID) {
        var arrvObj = {
          "Station" : arrivalEntry.Messages[c][3].Value,
...
//                    "Note" : "",
                ...
                if (arrivalEntry.TripList.Trips[c].Position === undefined) {
                  ...
                  /*
                   * This group is for terminal stations. Basically, don't
                   * store the different flags like brd, arr, etc. because they
                   * don't make sense in the scope of a terminal station. 
                   * For example: a trip to Oak Grove from Forest Hills with
                   * a countdown is how long until that train will leave the
                   * station, so it is probably already there and terms like
                   * "approaching" don't make any sense.
                   */

 ...
 * 
 * If a Commuter Rail Line is requested, then ALL Commuter Rail lines for
 * the given stop will be fetched. This was deemed helpful because of 
 * stations or lines on which multiple C.R. lines operate (often serving
 * the same platforms, which could be confusing. This situation  doesn't
 * exist on Boston's subway.
 * 
...
function reloadData( name, line )
{
  ...
  try {
    if (line == "red" || line == "orange" || line == "blue") {
      if (masstrac.getSubway( name, line ) == false) {
        ...
        $("#timeZone").html( "Data Access Error (try again in a minute)" );
        return;
      }
      ...
      var stobject = masstrac.lookupStation( name );
      if (stobject == null)
        return;
      
      /*
       * Load all the Commuter Rail JSONs into the arrivals array
       * but skip any subway trips.
       */
      for (var svcnum = 0; svcnum < stobject.svcs.length; svcnum++) {
        if (stobject.svcs[svcnum] == "red" ||
            stobject.svcs[svcnum] == "orange" ||
            stobject.svcs[svcnum] == "blue" ||
            stobject.svcs[svcnum] == "green")
          continue;
        
        if (masstrac.getCommuterRail( name, stobject.svcs[svcnum] ) == false) {
    ...
    $("#timeZone").html( "Data Processing Error (try again in a minute)" );
    return;
  }
  ...
  for (var c = 0; c < masstrac.arrivals.length; c++) {
    ...
    if (masstrac.arrivals[c].Type == "sch") {
      if (masstrac.arrivals[c].Countdown <= 60) {
        mt_arrivals.append("<li><a href='mtrip.html?name=" + name +
                           "&line=" + masstrac.arrivals[c].Line +
...
{
  // Some of this code is repeated from above, basically handle the errors
  // if any are present, and stop the processing if they are.
  $("#timeZone").html( "Fetching new data..." );
  masstrac.dumpData();
  $("#tripList").empty();
  try {
  ...
  for (var i = 0; i < masstrac.trips.length; ++i) {
    if (masstrac.trips[i].TripID == tripID) {
      ...
      /*
       * The new version of the Commuter Rail feed doesn't guarantee sort
       * based on sequence of stops. Ex: North Station could appear before
       * Back Bay on an inbound trip, which is confusing!
       */
      masstrac.trips[i].Predictions.sort( function (a, b) {
        return a.UNIXtime - b.UNIXtime;
      });
  
      // If there's a note in the trips array about the trip, insert it onto the
      // trip information page.
      if (masstrac.trips[i].Note !== "") {
        $("#noteZone").html( "Note: " + masstrac.trips[i].Note + "." );
      } else {
        $("#noteZone").html( "Note: -None-" );
      }
      ...
      for (var j = 0; j < masstrac.trips[i].Predictions.length; ++j) {
        $("#tripList").append( "<li><table><tr><td>" +
                               ...
                               "</td><td>" +
                               jGetAMPMTime( new Date(masstrac.trips[i].Predictions[j].UNIXtime * 1000),
                                             false, false ) +
                               "</td></tr></table></li>" );
      }
      break;     // Don't waste CPU cycles
    }
  }

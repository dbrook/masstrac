 ...
 * 
 * Last Updated by Daniel Brook on 2013-04-24 at 8:45 PM
 * 
 ...
 * 
 * Additional note: the plotting is done based on matching the arrival countdown
 * for the first object in the specific trips array element, and the heading
 * of the train itself. This is not an ideal situation, and it required some
 * workarounds to handle the terminal stations where trains stop and turnaround
 * (unlike at Ashmont, where they continue passing through and come back around
 * ... consequently Ashmont is the ONLY station that doesn't suffer from this).
 * 
 * The workaround implemented works fine, but if there are ever short-turn trips
 * then the bug will reveal itself for those (and these trips are difficult
 * to test against to implement a workaround).
 */
          ...
          switch (staObj.redMap.sec) {
            case "Cambridge":
              if (masstrac.trips[trip].heading >= 225 ||
                ...
                /*
                 * As it turns out, the way some stations are configured makes
                 * it so the heading variable is stuck facing outbound until
                 * the train starts moving the other direction. This fights the
                 * way we determine how to position the trip at the following
                 * terminal stations:
                 *     Alewife, Braintree, Oak Grove, and Forest Hills
                 * 
                 * It can be worked around with a simple if.
                 */
                if (staObj.name == "Alewife" &&
                    masstrac.trips[trip].Destination != "Alewife") {
                  canvops.drawTriangle( cxt, false, canvops.redSouthC, staObj.redMap.sta );
                  continue;        // Just plot the arrow and look at next trip
                }
            ...
            case "Braintree":
              if (masstrac.trips[trip].heading >= 270 ||
                ...
                /*
                 * Like with Alewife, Braintree also has the weird heading bug
                 * because of its configuration as a terminal station.
                 */
                if (staObj.name == "Braintree" &&
                    masstrac.trips[trip].Destination != "Braintree") {
                  canvops.drawTriangle( cxt, true, canvops.redNorthB, staObj.redMap.sta );
                  continue;        // Just plot the arrow and look at next trip
                }
      ...
      for (trip = 0; trip < masstrac.trips.length; ++trip) {
        if (masstrac.trips[trip].TripID.substring(0,1) == "B" &&
          ...
          if (masstrac.trips[trip].heading >= 350 ||
            ...
            /*
             * Like with Braintree, Wonderland also has the weird heading bug
             * because of its configuration as a terminal station.
             */
            if (staObj.name == "Wonderland" &&
                masstrac.trips[trip].Destination != "Wonderland") {
            ...
            /*
             * The terminal station plotting bug rears its ugly head after 
             * hours when Bowdoin Square isn't stopped at, so a little extra
             * handling is required.
             */
            if (staObj.name == "Government Center" &&
                (masstrac.trips[trip].Destination != "Bowdoin" &&
                 masstrac.trips[trip].Destination != "Government Center")) {
              canvops.drawTriangle( cxt, true, canvops.blueEast, staObj.blueMap.up );
              continue;        // Just plot the arrow and look at next trip
            } else if (staObj.name == "Bowdoin" &&
                       masstrac.trips[trip].Destination != "Bowdoin") {
              canvops.drawTriangle( cxt, true, canvops.blueEast, staObj.blueMap.sta );
              continue;
            }
      ...
      for (trip = 0; trip < masstrac.trips.length; ++trip) {
        if (masstrac.trips[trip].TripID.substring(0,1) == "O" &&
          ...
          if (masstrac.trips[trip].heading > 270 ||
            ...
            // North Bound
            /*
             * Oak Grove has the weird heading bug
             * because of its configuration as a terminal station.
             */
            if (staObj.name == "Oak Grove" &&
                masstrac.trips[trip].Destination != "Oak Grove") {
            ...
            // South Bound
            /*
             * Forest Hills has the weird heading bug
             * because of its configuration as a terminal station.
             */
            if (staObj.name == "Forest Hills" &&
                masstrac.trips[trip].Destination != "Forest Hills") {
              canvops.drawTriangle( cxt, true, canvops.orangeNorth, staObj.orangeMap.sta );
              continue;        // Just plot the arrow and look at next trip
            }
